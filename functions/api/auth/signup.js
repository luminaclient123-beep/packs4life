...
import { randomToken, hashPassword, sessionCookie, addLinkedToken } from "../../../lib/auth.js";
...
export async function onRequestPost(context) {
  ...
  const existingEmail = await env.PACKS_KV.get(emailKey);
  if (existingEmail) return badRequest("An account with that email already exists.", 409);

  const usernameKey = `username:${name.toLowerCase()}`;
  const existingName = await env.PACKS_KV.get(usernameKey);
  if (existingName) return badRequest("That name is already taken.", 409);
  ...
  await env.PACKS_KV.put(usernameKey, id);
  ...
  return new Response(JSON.stringify({ user: publicUser }), { status: 201, headers });
}
